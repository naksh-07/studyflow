...
import { subjects as subjectsTable } from '@/db/schema/subjects';
  ...
  static async getSnapshot(userId: string): Promise<StudyFlowSnapshot | null> {
    ...
    if (db) {
      ...
      for (const sub of subjectsResult) {
        // Handle record matching. subjectId inside sessions has "subject-" prefix removed in mapped results, 
        // but subjects table id is generated by subjectRecordId(subjectName) which does not use the "subject-" prefix.
        subjectConstraintsMap.set(sub.name, {
      ...
      const mappedOriginalSessions = timetableSessions.map(session => {
        const subName = session.subjectId ? session.subjectId.replace(/^subject-/, '') : null;
        const constraints = subName ? subjectConstraintsMap.get(subName) : undefined;
        return {
          id: session.id,
          subjectId: subName,
